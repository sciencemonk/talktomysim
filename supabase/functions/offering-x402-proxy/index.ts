...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type, x-402-payment, accept, user-agent',
...
serve(async (req) => {
  ...
  try {
    ...
    // Extract offeringId from path: /offering-x402-proxy/offering/{offeringId}
    ...
    console.log('[offering-x402-proxy] Request:', {
      method: req.method,
      path: url.pathname,
      offeringId,
    ...
    if (!offeringId) {
      return new Response(
        JSON.stringify({ 
          x402Version: 1, 
          error: 'offeringId required in path or query parameter' 
        }),
        { 
          status: 400, 
          headers: { 
    ...
    if (error || !offering) {
      console.log('[offering-x402-proxy] Offering not found:', error);
      return new Response(
        JSON.stringify({ 
          x402Version: 1, 
    ...
    console.log('[offering-x402-proxy] Offering found:', {
      id: offering.id,
    ...
    console.log('[offering-x402-proxy] Returning 402 response');
    ...
    console.error('[offering-x402-proxy] Error:', error);
    return new Response(
      JSON.stringify({
        x402Version: 1,
