...
Return ONLY the system prompt text, nothing else.`
          ...
          {
            role: "user",
            content: `Create a system prompt for an AI agent named "${name}" in the "${category}" category with this description: ${description}${integrations.length > 0 && category === 'crypto' ? `\n\nThis agent has access to the following integrations: ${integrations.map((id: string) => {
