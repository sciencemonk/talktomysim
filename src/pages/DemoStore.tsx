...
export default function DemoStore() {
  ...
  const handleShareLink = () => {
    const url = `https://solanainternetmarket.com/${mockXData.username}`;
  ...
  return <div className="min-h-screen bg-gradient-to-b from-background via-background to-muted/20 font-system">
      {/* Top Navigation Bar */}
      ...
      <div className="border-b border-border/40 bg-gradient-to-r from-card/95 via-card/80 to-card/95 backdrop-blur-sm relative overflow-hidden">
        <div className="container mx-auto px-4 md:px-6 py-8 md:py-12 relative z-10">
          <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-start md:items-center">
            {/* Profile Image */}
            <Avatar className="h-24 w-24 md:h-32 md:w-32 border-4 shadow-2xl ring-4" style={{
            borderColor: primaryColor,
            '--tw-ring-color': `${primaryColor}33`
          } as any}>
              <AvatarImage src={mockXData.profileImageUrl} alt={mockXData.displayName} className="object-cover" referrerPolicy="no-referrer" />
            ...
            <div className="flex-1 space-y-4">
              <div>
                ...
                <div className="flex items-center gap-3 flex-wrap mb-2">
                  ...
                  {mockXData.verified && <Badge variant="default" className="gap-1" style={{
                ...
                <div className="flex items-center gap-3 text-muted-foreground">
                  <button onClick={handleCopyUsername} className="flex items-center gap-2 hover:text-foreground transition-colors font-mono text-sm">
                    <span className="truncate max-w-[200px] md:max-w-xs">
                      {mockWallet}
                    </span>
                    {usernameCopied ? <Check className="h-4 w-4 text-green-500 shrink-0" /> : <Copy className="h-4 w-4 shrink-0" />}
                  </button>
                  <a href={`https://x.com/${mockXData.username.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-foreground transition-colors">
