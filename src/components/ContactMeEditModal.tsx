...
const ContactMeEditModal = ({
  open,
  onOpenChange,
  simId,
  editCode
}: ContactMeEditModalProps) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const {
        data: verifyData,
        error: verifyError
      } = await supabase.from('advisors').select('edit_code, name').eq('id', simId).maybeSingle();
      if (verifyError) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const {
        error
      } = await supabase.from('advisors').delete().eq('id', simId);
      if (error) throw error;
      await queryClient.invalidateQueries({
        queryKey: ['user-sim']
      });
      await queryClient.invalidateQueries({
        queryKey: ['user-sim-check']
      });
      await queryClient.invalidateQueries({
        queryKey: ['my-sim-conversations']
      });
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto p-0">
          <div className="p-8 space-y-6">
            ...
            {!editCode && <div className="space-y-2">
                <Label htmlFor="editCode" className="text-sm font-medium">
                  Edit Code <span className="text-destructive">*</span>
                </Label>
                <Input id="editCode" value={inputEditCode} onChange={e => setInputEditCode(e.target.value)} placeholder="Enter 6-digit edit code" maxLength={6} className="h-11 bg-background font-mono text-center text-lg tracking-widest" required />
              ...
              <TabsContent value="settings" className="mt-6 space-y-6">
                ...
                handleSave();
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-6 items-start">
                      <div className="flex flex-col items-center gap-3">
                        <Avatar className="w-24 h-24 cursor-pointer border border-border/50 hover:border-border transition-colors" onClick={() => fileInputRef.current?.click()}>
                          {avatarPreview ? <AvatarImage src={avatarPreview} alt="Avatar preview" className="object-cover" /> : <AvatarFallback className="bg-muted/50">
                      ...
                      <div className="flex-1 space-y-4">
                        <div className="space-y-2">
                          ...
                          <Input id="name" value={name} onChange={e => setName(e.target.value)} placeholder="Sim name" maxLength={50} required className="bg-stone-950" />
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea id="description" value={description} onChange={e => setDescription(e.target.value)} placeholder="Describe your contact form" rows={4} required />
                  </div>

                  {/* Welcome Message */}
                  <div className="space-y-2">
                    <Label htmlFor="welcome">Welcome Message</Label>
                    <Textarea id="welcome" value={welcomeMessage} onChange={e => setWelcomeMessage(e.target.value)} placeholder="Message shown above the contact form" rows={3} />
                  </div>

                  {/* Action Buttons */}
                  <div className="flex gap-3 pt-4 border-t">
                    <Button type="button" variant="destructive" onClick={() => setShowDeleteDialog(true)} disabled={isSaving || isDeleting}>
